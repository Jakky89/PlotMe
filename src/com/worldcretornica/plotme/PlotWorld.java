...
public class PlotWorld implements Comparable<PlotWorld>
{
	...
	public PlotWorld(int pmWorldId, String pmWorldName)
	{
		id = pmWorldId;
		worldName = pmWorldName;
		plotPositions = new HashMap<PlotPosition, Plot>();
		ProtectedBlocks = new HashSet<Jakky89ItemIdData>();
		PreventedItems = new HashSet<Jakky89ItemIdData>();
		
		RoadHeight			= PlotMe.DEFAULT_ROAD_HEIGHT;
		PlotSize			= PlotMe.DEFAULT_PLOT_SIZE;
		PlotAutoLimit		= PlotMe.DEFAULT_PLOT_AUTO_LIMIT;
		PathWidth			= PlotMe.DEFAULT_PATH_WIDTH;
		
		
		BottomBlock 		= PlotMe.DEFAULT_ROAD_MAIN_BLOCK;
		WallBlock			= PlotMe.DEFAULT_WALL_BLOCK;
		ForSaleWallBlock	= PlotMe.DEFAULT_FORSALE_BLOCK;
		AuctionWallBlock	= PlotMe.DEFAULT_AUCTION_BLOCK;
		ProtectedWallBlock	= PlotMe.DEFAULT_PROTECT_BLOCK;
		PlotFloorBlock		= PlotMe.DEFAULT_FLOOR_BLOCK;
		PlotFillingBlock	= PlotMe.DEFAULT_FILL_BLOCK;
		RoadMainBlock		= PlotMe.DEFAULT_ROAD_MAIN_BLOCK;
		RoadStripeBlock		= PlotMe.DEFAULT_ROAD_STRIPE_BLOCK;
		
		spawnX = 0;
		spawnY = this.RoadHeight;
		spawnZ = 0;
	}
	...
	public Location getCenterBlockLocation(Plot plot)
	{
		if (plot != null && plot.getPlotWorld().equals(this)) {
			...
			return new Location(this.getMinecraftWorld(), ((double)baseLocation.getBlockX() + (double)(PlotSize / 2)), RoadHeight, ((double)baseLocation.getBlockZ() + (double)(PlotSize / 2)));
		}
		return null;
	}
	...
	{
		if (loc != null && loc.getWorld().getName().equals(this.getName())) {
	...
	{
		if (blockState != null && blockState.getWorld().equals(this.getName())) {
	...
	{
		if (block != null && block.getWorld().equals(this.getName())) {
	 ...
	 * TODO: NEEDS TESTING!!!
	 */
	public Plot getPlotAtLocation(double posX, double posZ)
	{
		double divi = getPlotBlockPositionMultiplier();
	...
	public Plot getPlotAtLocation(Location loc)
	{
		...
		{
			return getPlotAtLocation(loc.getBlockX(), loc.getBlockZ());
		}
		return null;
	}
	...
	public Location getMinBlockLocation(Plot plot)
	{
		...
		return new Location(this.getMinecraftWorld(), locx, 1, locz);
	}
	
	public Location getMaxBlockLocation(Plot plot)
	{
		World minecraftWorld = this.getMinecraftWorld();
	...
	public Block getCenterBlock(int plotX, int plotZ)
	{
		...
		double multi = getPlotBlockPositionMultiplier();
		return minecraftWorld.getBlockAt((int)Math.round((double)(plotX * multi) + (double)(PlotSize / 2)), RoadHeight, (int)Math.round((double)(plotZ * multi) + (double)(PlotSize / 2)));
	}
	...
	{
		if (loc1.getWorld().getName().isEmpty() || loc2.getWorld().getName().isEmpty() || !loc1.getWorld().getName().equals(loc2.getWorld().getName())) {
			return null;
		}
	...
	{
		if (plot == null || !plot.getPlotWorld().getName().equals(this.getName()))
		{
			return null;
		}
